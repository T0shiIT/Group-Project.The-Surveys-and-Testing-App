    ...
    srv.Get("/login_request", [](const httplib::Request& req, httplib::Response& res){
        ...
        if (type == "github"){
            url = "https://github.com/login/oauth/authorize?client_id=" + GH_ID + 
                   "&redirect_uri=https://paint-geek-broken-mls.trycloudflare.com/auth/oauth/github" +
                   "&state=" + state;
        }
        else if (type == "yandex"){
            url = "https://oauth.yandex.ru/authorize?response_type=code&client_id=" + YAN_ID + 
                   "&redirect_uri=https://paint-geek-broken-mls.trycloudflare.com/auth/oauth/yandex" +
                   "&state=" + state;
        }
        if (!url.empty()){
