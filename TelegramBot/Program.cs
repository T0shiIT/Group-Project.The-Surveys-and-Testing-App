...
//---[НАСТРОЙКИ NGROK]---
string ngrokUrl = Environment.GetEnvironmentVariable("NGROK_URL") ?? "https://vision-oct-jewish-eleven.trycloudflare.com";
...
Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] 🌐 Ngrok URL: {ngrokUrl}");
...
while (true) {
    try {
        ...
        await CheckAuthorizationStatus(bot, db, ngrokUrl);
            ...
            try {
                ...
                if (chatType == ChatType.Private) {
                    switch (text) {
                        ...
                        case "GitHub" when string.IsNullOrEmpty(accessToken):
                            await HandleAuthChoice(bot, db, chatId, userId, "github", ngrokUrl);
                            continue;
                        case "Yandex" when string.IsNullOrEmpty(accessToken):
                            await HandleAuthChoice(bot, db, chatId, userId, "yandex", ngrokUrl);
                            continue;
                    }
                }
                ...
                switch (lowerText) {
                    ...
                    case "/help":
                        ...
                        helpText += "👤 Для ЛС:\n/start, /help, /reset\n";
                        helpText += "👥 Для групп:\n/start@{botInfo.Username}, /help, /reset";
                        await bot.SendTextMessageAsync(
                            chatId: chatId,
...
async Task HandleAuthChoice(ITelegramBotClient bot, IDatabase db, long chatId, long userId, string provider, string ngrokUrl) {
    ...
    try {
        var response = await httpClient.GetAsync($"{ngrokUrl}/auth/login_request?type={authType}&state={state}");
...
async Task CheckAuthorizationStatus(ITelegramBotClient bot, IDatabase db, string ngrokUrl) {
    try {
        ...
        foreach (var key in keys) {
            ...
            try {
                ...
                var response = await client.GetAsync($"{ngrokUrl}/auth/check_state?state={state}");
                    ...
                    try {
                        ...
                        if (!string.IsNullOrEmpty(accessToken)) {
                            ...
                            string successMessage = $"🎉 Поздравляем, {userName}!\n\n{providerIcon} Авторизация через {providerName} прошла успешно!\n\nТеперь вы можете:\n" +
                                                   "📚 Пройти тесты по программированию\n" +
                                                   "📊 Проверить свои результаты\n" +
                                                   "👥 Создать опрос для команды\n\n" +
                                                   "Что хотите сделать первым делом?";
                            
                            // Создаем клавиатуру с быстрыми действиями
                            var quickActionsKeyboard = new ReplyKeyboardMarkup(new[]
                            {
                                new KeyboardButton("📚 Пройти тест"),
                                new KeyboardButton("📊 Мои результаты"),
                                new KeyboardButton("👥 Создать опрос")
                            })
                            {
                                ResizeKeyboard = true,
